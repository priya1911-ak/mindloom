/* ============================================
   MindLoom â€“ Rise Above the Weight Within
   Core JavaScript
   ============================================ */
...
/* ==========================================
   FLOATING PARTICLES (Hero / Backgrounds)
   ========================================== */
...
function initMoodTracker() {
  ...
  const moods = [
    ...
    { emoji: 'ðŸ˜', label: 'Neutral', color: '#a8d8ea' },
    { emoji: 'ðŸ˜”', label: 'Sad', color: '#6c88c4' },
    { emoji: 'ðŸ˜¡', label: 'Angry', color: '#ff6b6b' },
    { emoji: 'ðŸ˜°', label: 'Anxious', color: '#c9b1ff' },
    { emoji: 'ðŸ˜Œ', label: 'Calm', color: '#7ec8a0' }
  ];
  ...
  moods.forEach(mood => {
    ...
    btn.addEventListener('click', () => saveMood(mood));
  ...
  function renderHabits() {
    ...
    list.innerHTML = habits.map(h => `
      <div class="habit-item ${h.completed ? 'completed' : ''}">
        ...
        <span class="habit-streak">ðŸ”¥ ${h.streak} day${h.streak !== 1 ? 's' : ''}</span>
        <button class="habit-delete" data-id="${h.id}">âœ•</button>
      </div>
    `).join('');
...
function initBreathingExercise() {
  ...
  function breathCycle() {
    ...
    guide.textContent = 'Inhaleâ€¦';

    setTimeout(() => {
      if (!isRunning) return;
      // Hold - 4s
      circle.className = 'breathing-circle hold';
      guide.textContent = 'Holdâ€¦';

      setTimeout(() => {
        if (!isRunning) return;
        // Exhale - 4s
        circle.className = 'breathing-circle exhale';
        guide.textContent = 'Exhaleâ€¦';
...
function initRelaxPage() {
  ...
  // Create floating particles for relax page
  for (let i = 0; i < 30; i++) {
    const p = document.createElement('div');
  ...
  if (musicBtn) {
    let isPlaying = false;
    // Create an audio context for ambient sound
    let audioCtx = null;
    let oscillator = null;
    let gainNode = null;
    ...
    musicBtn.addEventListener('click', () => {
      if (!isPlaying) {
        ...
        oscillator.frequency.setValueAtTime(174, audioCtx.currentTime); // Solfeggio frequency for relaxation
        gainNode.gain.setValueAtTime(0.05, audioCtx.currentTime);
        oscillator.connect(gainNode);
        gainNode.connect(audioCtx.destination);
        oscillator.start();
        isPlaying = true;
        musicBtn.textContent = 'ðŸ”‡ Pause Sound';
      } else {
        if (oscillator) oscillator.stop();
        if (audioCtx) audioCtx.close();
        isPlaying = false;
        musicBtn.textContent = 'ðŸ”Š Play Ambient Sound';
      }
    });
  }

  // Cycle through relaxing messages
  const messages = [
    'Let your thoughts float awayâ€¦',
    'You are lighter than your worriesâ€¦',
    'Breathe in peace, exhale tensionâ€¦',
    'This moment is yours. Be stillâ€¦',
    'You are safe. You are enoughâ€¦'
  ];
